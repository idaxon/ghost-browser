/**
 * GhostStack User Agent Rotator
 * Maintains 100 real UA strings, rotates per session.
 * @module UserAgentRotator
 */
import { createHash, randomBytes } from 'crypto'

const UA_STRINGS_WIN = [
...
export class UserAgentRotator {
  ...
  rotate(): void {
    const pool = process.platform === 'darwin' ? UA_STRINGS_MAC : UA_STRINGS_WIN
    this.sessionUA = pool[Math.floor(Math.random() * pool.length)]
  }
}
